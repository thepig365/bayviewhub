...
export function routeLabelForChineseFallback(pathname: string): string {
  ...
  if (englishPath === '/newsletter') return 'Newsletter'
  if (englishPath.startsWith('/backyard-small-second-home')) return 'Backyard Small Second Home'
  if (englishPath === '/about') return 'About'
  if (englishPath === '/visit') return 'Visit'
  if (englishPath === '/events') return 'Events'
  if (englishPath.startsWith('/partners')) return 'Partners'
  if (englishPath === '/workshops') return 'Workshops'
  if (englishPath === '/edible-gardens') return 'Edible Gardens'
  return englishPath
}
